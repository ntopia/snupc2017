...
\begin{frame}
  \frametitle{Div1K. 정육면체를 사랑하는 사람}
  \begin{itemize}
    \item $K^3 < N \leq (K+1)^3$ 을 만족하는 $K$를 잡아봅니다.
    \item 즉, 한 변이 $K+1$인 큐브에는 전부 다 넣어 정리할 수 있는 것이죠.
    \item 따라서 우선 겉넓이가 $6(K+1)^2$ 인 해가 존재하게 됩니다.
    \item 이보다 더 좋은 해를 찾아봅시다!
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Div1K. 정육면체를 사랑하는 사람}
  \begin{itemize}
    \item 직육면체의 각 변을 $A, B, C$ 라 하고 $A \leq B \le C$ 를 만족한다 합시다.
    \item $N \leq A \cdot B \cdot C$ 이므로 $N/A \leq B \cdot C$
    \item $B + C \geq 2 \sqrt{B C}$   (산술-기하 평균 부등식)
    \item 겉넓이는 $2 (A B + B C + C A) = 2 A (B + C) + 2 B C$
  \end{itemize}
\end{frame}
